...
Controls an **already-provisioned** Keon device from Node.js over the Feel
Unified Gateway (FUG) REST transport. No browser, no Web Bluetooth, no OAuth —
just a `deviceConnectionKey` and HTTP. This is the clearest demonstration that
...
After connecting it pulls the device status once (`forceStatusReport`), then
polls every 2 seconds while sending a few commands, then stops and disconnects.
